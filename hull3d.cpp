...
void draw_hull(int k){
  ...
  if (colors.size() < k) {
    vector<GLfloat> nc;
    nc.push_back((double)rand() / (double)((unsigned)RAND_MAX + 1));
    nc.push_back((double)rand() / (double)((unsigned)RAND_MAX + 1));
    nc.push_back((double)rand() / (double)((unsigned)RAND_MAX + 1));
    nc.push_back(0.5);
    colors.push_back(nc);
  ...
  if (hull.size() >0) {
    ...
    for (i=0; i<k; i++) {
      ...
      glVertex3f(windowtoscreen(hull[i].p1->x), windowtoscreen(hull[i].p1->y), windowtoscreen(hull[i].p1->z));

      glVertex3f(windowtoscreen(hull[i].p2->x), windowtoscreen(hull[i].p2->y), windowtoscreen(hull[i].p2->z));

      glVertex3f(windowtoscreen(hull[i].p3->x), windowtoscreen(hull[i].p3->y), windowtoscreen(hull[i].p3->z));
      glPopMatrix();
